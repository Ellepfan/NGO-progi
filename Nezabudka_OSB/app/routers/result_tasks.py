...
async def update_result(result_id: int, name_user: str, db: DbSession):
    ...
    data_now = str(datetime.now().date())
    data_now = data_now[8:10] + "." + data_now[5:7] + "." + data_now[:4]
    result = db.execute(text("SELECT * FROM result WHERE id = :id"), {"id": result_id}).fetchone()
    if not result:
        raise HTTPException(status_code=404, detail="Result not found")

    # Преобразуем data_task из строки в дату
    try:
        data_task_date = datetime.strptime(result.data_task, "%d.%m.%Y").date()
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid date format in data_task")

    if data_task_date > datetime.now().date():
        return {"detail": "Task date is in the future. No update performed."}
