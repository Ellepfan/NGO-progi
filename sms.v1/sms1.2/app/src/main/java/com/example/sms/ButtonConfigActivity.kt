...
class ButtonConfigActivity : AppCompatActivity() {
    private lateinit var objectNameEditText: EditText
    ...
    private fun loadButtonConfig() {
        ...
        configs.find { it.id == buttonId }?.let { config ->
            objectNameEditText.setText(config.objectName)
    ...
    private fun saveButtonConfig() {
        val objectName = objectNameEditText.text.toString().trim()
        val name = nameEditText.text.toString().trim()
        val phone = phoneEditText.text.toString().trim()
        val message = messageEditText.text.toString().trim()

        if (objectName.isEmpty() || name.isEmpty() || phone.isEmpty() || message.isEmpty()) {
